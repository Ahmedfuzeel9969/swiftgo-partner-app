...
import { validateRideLocationFix } from "./ride-location-contract.mjs";
...
export function buildP2pLocationMessage(input, ctx) {
  ...
  const checked = validateRideLocationFix({
    ...input, lat, lng, observedAt,
    sequence: input?.sequence ?? seq,
    trackingSessionId: input?.trackingSessionId ?? input?.sessionId ?? trackingSessionId,
    accuracyM: input?.accuracyM ?? input?.accuracy ?? null,
    headingDeg: input?.headingDeg ?? input?.heading ?? null,
    speedMps: input?.speedMps ?? input?.speed ?? null,
  ...
  const msg = {
    ...
    lat,
    lng,
    fixSequence: checked.fix.sequence,
    sampleSessionId: checked.fix.trackingSessionId,
    ...(ctx.rideId ? { rideId: ctx.rideId } : {}),
    ...(ctx.assignmentId ? { assignmentId: ctx.assignmentId } : {}),
    accuracyM: checked.fix.accuracyM,
    headingDeg: checked.fix.headingDeg,
    speedMps: checked.fix.speedMps,
...
export function validateP2pMessage(raw, auth = {}) {
  ...
  // Packet ordering and original GPS sample ordering are separate.
  const seq = obj.seq;
  if (!Number.isSafeInteger(seq) || seq < 1) return { ok: false, reason: "invalid_sequence" };
  const last = auth.lastSequence || 0;
  if (seq === last) return { ok: false, reason: "duplicate_sequence" };
  if (seq < last) return { ok: false, reason: "decreasing_sequence" };
  if (auth.rideId && obj.rideId !== auth.rideId) return { ok: false, reason: "wrong_ride" };
  if (auth.assignmentId && obj.assignmentId !== auth.assignmentId) return { ok: false, reason: "wrong_assignment" };
  const sampleSessionId = obj.sampleSessionId ?? obj.trackingSessionId;
  if (obj.role === "driver" && sampleSessionId !== obj.trackingSessionId) {
    return { ok: false, reason: "wrong_tracking_session" };
  }
  const checked = validateRideLocationFix({
    ...obj, sequence: obj.fixSequence ?? seq, trackingSessionId: sampleSessionId,
  }, { nowMs: auth.nowMs ?? Date.now(), previous: auth.previousFix });
  if (!checked.ok) return checked;
  return {
    ok: true, type, message: obj,
    fix: { ...checked.fix, source: "p2p", peerSessionId: obj.peerSessionId, transportSequence: seq },
  };
}
