/**
 * Canonical shared implementation — remediation phase three.
...
export function createP2pPeerSession(deps) {
  ...
  let fallbackAfterMs = Math.min(60_000, Math.max(5_000, Number(deps.fallbackAfterMs) || P2P_FALLBACK_AFTER_MS));
  let degradedAfterMs = Math.min(P2P_DEGRADED_AFTER_MS, Math.floor(fallbackAfterMs * 0.75));
  const Peer =
  ...
  let lastInboundLocAt = null;
  let lastOutboundLocAt = null;
  let lastReceivedFix = null;
  let lastAckAt = null;
  let lastLocAckAt = null;
  let lastHbAt = null;
  ...
  let iceRestartTimer = 0;
  let iceRefreshTimer = 0;
  let runtimeIceConfig = null;
  let firstAckTimer = 0;
  ...
  function isCurrent(gen = generation) {
    return !closed && gen === generation && (typeof deps.isCurrentGeneration !== "function" || deps.isCurrentGeneration(gen));
  }

  function clearTimers() {
    for (const cancel of [...pendingAsync]) cancel();
    clearT(iceRefreshTimer); iceRefreshTimer = 0;
    if (healthTimer) {
      clearT(healthTimer);
      healthTimer = 0;
    }
  ...
  function validateDriverAck(msg) {
    const seq = msg?.seq;
    if (!Number.isSafeInteger(seq) || seq < 1) return { ok: false, reason: "invalid_sequence" };
    ...
    if (nowMs() - sentFixTimes.get(seq) > fallbackAfterMs) return { ok: false, reason: "stale_ack" };
    return { ok: true, seq };
  }
  ...
  function pruneSentSequences() {
    while (sentSequences.size > P2P_MAX_SENT_SEQUENCES_RETAINED) {
      ...
      sentFixTimes.delete(oldest);
    }
  }

  function trackSentSequence(seq, observedAt) {
    sentSequences.add(seq);
    sentFixTimes.set(seq, observedAt);
    pruneSentSequences();
  }

  function releaseSentSequence(seq) {
    sentSequences.delete(seq);
    sentFixTimes.delete(seq);
  }

  function classifyInboundAck(msg) {
    // Location health requires an explicit loc ACK. Treating a legacy/untyped
    // transport ACK as delivery proof can keep Firebase sparse while the
    // customer marker is actually frozen.
    return classifyAckKind(msg) || "";
  }

  function isTransportAliveNow() {
    if (!isLinkOpen()) return false;
    const vals = [lastAckAt, lastHbAt, lastInboundLocAt]
      .filter((v) => v != null && Number.isFinite(Number(v)))
      .map((v) => Number(v));
    if (!vals.length) return false;
    return nowMs() - Math.max(...vals) <= fallbackAfterMs;
  }

  function isLocDeliveryHealthyNow() {
    if (assignmentVersion < 1 || !isLinkOpen()) return false;
    if (role === "customer") {
      const fixAge = lastInboundLocAt != null ? nowMs() - lastInboundLocAt : Infinity;
      return fixAge <= degradedAfterMs;
    }
    const ackAge = lastLocAckAt != null ? nowMs() - lastLocAckAt : Infinity;
    return (
      channel?.readyState === "open" &&
      lastSequenceSent > 0 &&
      lastValidFixAt != null &&
      ackAge <= degradedAfterMs
    );
  }
  ...
  function scheduleFirstAckTimeout(gen) {
    if (firstAckTimer || lastLocAckAt != null) return;
    firstAckTimer = setT(() => {
      firstAckTimer = 0;
    ...
    }, Math.min(P2P_FIRST_ACK_TIMEOUT_MS, fallbackAfterMs));
  }
  ...
  function isLinkOpen() {
    return channel?.readyState === "open" &&
      !["failed", "disconnected", "closed"].includes(String(pc?.connectionState || "")) &&
      !["failed", "disconnected", "closed"].includes(String(pc?.iceConnectionState || ""));
  }
  ...
  function evaluateHealth() {
    ...
    const now = nowMs();
    if (!isLinkOpen()) {
      setState(P2P_STATE.FIREBASE_FALLBACK, "channel_down");
      if (!iceRestartTimer) requestReconnect("channel_down");
      return;
    }
    if (role === "customer") {
      const fixAge = lastInboundLocAt != null ? now - lastInboundLocAt : Infinity;
      if (fixAge <= degradedAfterMs) {
        setState(P2P_STATE.P2P_HEALTHY);
      } else if (fixAge <= fallbackAfterMs) {
        setState(P2P_STATE.P2P_DEGRADED);
      } else {
        setState(P2P_STATE.FIREBASE_FALLBACK, "customer_activity_timeout");
        if (channelOpenedAt != null && now - channelOpenedAt >= fallbackAfterMs && !isTransportAliveNow()) {
          requestReconnect("customer_transport_timeout");
        }
      }
      return;
    }
    ...
    if (
      ...
      if (hasUnacknowledgedLoc || !isTransportAliveNow()) requestReconnect("driver_location_ack_timeout");
  ...
  function handleMessage(raw, gen) {
    ...
    if (validated.type === "loc" && validated.fix) {
      // Downstream rejection must not create an ACK or a healthy-delivery proof.
      if (deps.onLocationFix?.(validated.fix) === false) {
        counters.invalidMessages += 1;
        return;
      }
      lastSequenceRecv = validated.message.seq;
      lastReceivedFix = validated.fix;
      lastInboundLocAt = nowMs();
      ...
      }
      {
        const ack = buildP2pAckMessage({
          ...
          trackingSessionId,
          assignmentVersion,
          sequence: validated.message.seq,
          ackKind: P2P_ACK_KIND.LOC,
        });
        if (ack.ok && trySend(ack.serialized)) {
  ...
  function wireChannel(ch, gen) {
    if (!isCurrent(gen)) { ch.close?.(); return; }
    channel = ch;
    ...
    ch.onclose = () => {
      if (!isCurrent(gen) || channel !== ch || suppressingCloseReconnect) return;
    ...
    ch.onerror = () => {
      if (!isCurrent(gen) || channel !== ch) return;
      pushPipeline(
        "datachannel_error",
  ...
  async function waitIceComplete(peer, gen = generation) {
    ...
    await new Promise((resolve) => {
      ...
      pendingAsync.add(cancel);
      peer.addEventListener("icegatheringstatechange", done);
      timer = setT(() => finish({ timedOut: true }), 12_000);
      done();
    }).then((result) => {
      if (result?.cancelled || !isCurrent(gen) || pc !== peer) return;
      ...
      if (timedOut && gatheringState !== "complete") {
        pushPipeline(
          "ice_gather_timeout",
          {
            ...
            timedOut: true,
            candidates,
            failureReason: "ice_gather_timeout_12s",
          },
          P2P_DIAG.PIPELINE_ICE_GATHER_TIMEOUT
        );
        // Non-trickle signaling cannot deliver candidates gathered after upload.
        // Retry with fresh credentials instead of silently publishing host-only SDP.
        throw new Error("ICE_GATHER_TIMEOUT");
      } else {
        pushPipeline(
          "ice_gather_done",
  ...
  function wirePcLifecycle(peer, gen) {
    ...
    peer.onconnectionstatechange = () => {
      ...
      emitSignaling();
      if (st === "connected") {
        if (iceRestartTimer) clearT(iceRestartTimer);
        iceRestartTimer = 0;
        iceRestartAttempted = false;
        void pipe.captureSelectedPair("pc_connected");
      }
      if (st === "failed") {
        void pipe.captureSelectedPair("pc_failed");
        setState(P2P_STATE.FIREBASE_FALLBACK, "pc_connection_failed");
        if (role === "driver") void attemptIceRestart(gen);
        else requestReconnect("ice_failed");
    ...
    peer.oniceconnectionstatechange = () => {
      ...
      if (st === "failed") {
        ...
        if (role === "driver") void attemptIceRestart(gen);
        else requestReconnect("ice_conn_failed");
      } else if (st === "disconnected") {
        void attemptIceRestart(gen);
      }
    };
  ...
  async function attemptIceRestart(gen) {
    if (!isCurrent(gen) || closed || !pc) return false;
    ...
    setState(P2P_STATE.FIREBASE_FALLBACK, "ice_connection_interrupted");
    // The answerer creates a fresh PC. Rotate both SCTP/DTLS generations instead
    // of pairing a new answerer with an old driver association. Brief network
    // handovers may recover within the grace period without any new signaling.
    const failed = pc.connectionState === "failed" || pc.iceConnectionState === "failed";
    iceRestartTimer = setT(() => {
      iceRestartTimer = 0;
      if (!isCurrent(gen)) return;
      if (isLinkOpen()) { iceRestartAttempted = false; evaluateHealth(); return; }
      requestReconnect("ice_rotation_required");
    }, failed ? 1 : 3000);
    return true;
  }
  ...
  function createPc(gen) {
    ...
    if (runtimeIceConfig?.turnExpiresAtMs && runtimeIceConfig.turnExpiresAtMs <= nowMs()) {
      const { turn, turnExpiresAtMs, ...safe } = runtimeIceConfig;
      runtimeIceConfig = safe;
    }
    const ice = deps.iceConfig || resolveIceConfiguration(runtimeIceConfig ? { __SWIFTGO_P2P_ICE__: runtimeIceConfig } : undefined, nowMs());
    pipe.setIceMeta({
  ...
  async function loadIceConfiguration(gen) {
    if (typeof deps.ensureIceConfiguration !== "function") return;
    const config = await new Promise((resolve) => {
      let timer, settled = false;
      const finish = (value) => {
        if (settled) return; settled = true; clearT(timer); pendingAsync.delete(cancel); resolve(value);
      };
      const cancel = () => finish(null); pendingAsync.add(cancel);
      timer = setT(cancel, 6000);
      Promise.resolve().then(() => deps.ensureIceConfiguration({ rideId: deps.rideId || "", assignmentId: deps.assignmentId || "" }))
        .then(finish, () => finish(null));
    });
    if (isCurrent(gen) && config && typeof config === "object") runtimeIceConfig = config;
  }

  function boundedOperation(operation, gen, timeoutMs = 15000) {
    return new Promise((resolve, reject) => {
      let timer, settled = false;
      const finish = (value, error) => {
        if (settled) return; settled = true; clearT(timer); pendingAsync.delete(cancel);
        if (error && isCurrent(gen)) reject(error); else resolve(value);
      };
      const cancel = () => finish(null); pendingAsync.add(cancel);
      timer = setT(() => finish(null, new Error("P2P_OPERATION_TIMEOUT")), timeoutMs);
      Promise.resolve().then(() => isCurrent(gen) ? operation() : null).then((value) => finish(value), (error) => finish(null, error));
    });
  }

  function scheduleIceRefresh(gen, peer, retry = false) {
    clearT(iceRefreshTimer); iceRefreshTimer = 0;
    const expiry = Number(runtimeIceConfig?.turnExpiresAtMs);
    if (!Number.isFinite(expiry) || !isCurrent(gen)) return;
    const delay = retry ? 30000 : Math.max(1000, expiry - nowMs() - 60000);
    iceRefreshTimer = setT(async () => {
      iceRefreshTimer = 0;
      if (!isCurrent(gen) || pc !== peer) return;
      await loadIceConfiguration(gen);
      if (!isCurrent(gen) || pc !== peer) return;
      const next = Number(runtimeIceConfig?.turnExpiresAtMs);
      if (next > expiry && runtimeIceConfig?.turn) {
        try {
          peer.setConfiguration?.({ iceServers: resolveIceConfiguration({ __SWIFTGO_P2P_ICE__: runtimeIceConfig }, nowMs()).iceServers });
          scheduleIceRefresh(gen, peer);
          return;
        } catch { /* Retry with the existing allocation only until its expiry. */ }
      }
      if (nowMs() >= expiry) {
        setState(P2P_STATE.FIREBASE_FALLBACK, "turn_credentials_expired");
        requestReconnect("turn_credentials_expired");
      } else {
        // Preserve the previous deadline if the lookup returned no credentials.
        runtimeIceConfig = { ...runtimeIceConfig, turnExpiresAtMs: expiry };
        scheduleIceRefresh(gen, peer, true);
      }
    }, delay);
  }
  ...
  async function startAsDriver(meta = {}) {
    ...
    lastSequenceRecv = 0;
    lastValidFixAt = null;
    lastInboundLocAt = null;
    lastOutboundLocAt = null;
    lastReceivedFix = null;
    lastAckAt = null;
    lastLocAckAt = null;
    lastHbAt = null;
    lastOutboundAt = null;
    ...
    wireChannel(ch, gen);
    const offer = await boundedOperation(() => peer.createOffer(), gen);
    if (!isCurrent(gen) || peer !== pc) return null;
    await boundedOperation(() => peer.setLocalDescription(offer), gen);
    if (!isCurrent(gen) || peer !== pc) return null;
    pushPipeline(
      "offer_created",
      {
    ...
    await waitIceComplete(peer, gen);
    if (!isCurrent(gen)) return null;
    ...
    await boundedOperation(() => deps.onLocalDescription?.("offer", sdp, {
      peerSessionId,
      trackingSessionId,
      assignmentVersion,
      generation: gen,
    ...
    if (!isCurrent(gen) || peer !== pc) return null;
    if (!channelEverOpened) setState(P2P_STATE.CONNECTING);
  ...
  async function acceptRemoteAnswer(sdp, gen = generation) {
    ...
    const peer = pc;
    try {
      await boundedOperation(() => peer.setRemoteDescription({ type: "answer", sdp: text }), gen);
      if (!isCurrent(gen) || peer !== pc) return false;
  ...
  async function startAsCustomer(meta = {}) {
    ...
    peer.ondatachannel = (ev) => {
      if (!isCurrent(gen) || peer !== pc) { ev.channel?.close?.(); return; }
      if (ev.channel?.label !== P2P_DATA_CHANNEL_LABEL || (channel && channel !== ev.channel)) {
        ev.channel?.close?.(); return;
      }
      pushPipeline(
    ...
    await boundedOperation(() => peer.setRemoteDescription({ type: "offer", sdp: offerSdp }), gen);
    if (!isCurrent(gen) || peer !== pc) return null;
    ...
    const answer = await boundedOperation(() => peer.createAnswer(), gen);
    if (!isCurrent(gen) || peer !== pc) return null;
    await boundedOperation(() => peer.setLocalDescription(answer), gen);
    if (!isCurrent(gen) || peer !== pc) return null;
    pushPipeline(
      "answer_created",
      {
    ...
    await boundedOperation(() => deps.onLocalDescription?.("answer", sdp, {
      peerSessionId,
      trackingSessionId,
      assignmentVersion,
      generation: gen,
  ...
  function flushPendingLoc(gen = generation) {
    ...
    const now = nowMs();
    const minGapMs = P2P_MIN_LOC_GAP_MS;
    if (lastOutboundLocAt != null && now - lastOutboundLocAt < minGapMs && lastSequenceSent > 0) {
      scheduleCadenceFlush(gen, minGapMs - (now - lastOutboundLocAt));
      return;
    }
    ...
    trackSentSequence(nextSeq, built.message.observedAt);
    if (trySend(built.serialized, { countBackpressure: false })) {
      ...
      lastValidFixAt = now;
      lastOutboundLocAt = now;
      lastOutboundAt = now;
      pendingLoc = null;
      pendingLocGen = 0;
      if (prevSequenceSent === 0) {
  ...
  function scheduleReconnect(startFn) {
    ...
    const run = () => {
      ...
      Promise.resolve().then(fn).catch(() => {
        if (closed) return;
        setState(P2P_STATE.FIREBASE_FALLBACK, "reconnect_attempt_failed");
  ...
  function getState() {
    return {
      ...
      isOutboundLocationHealthy: Boolean(isLinkOpen() &&
        lastLocAckAt != null && nowMs() - lastLocAckAt <= fallbackAfterMs),
      lastInboundLocAt,
      lastOutboundLocAt,
      lastFallbackReason,
      firstPacketOut,
  ...
  function configureHealthPolicy(policy = {}) {
    if (policy.fallbackAfterMs == null) return;
    fallbackAfterMs = Math.min(60_000, Math.max(5_000, Number(policy.fallbackAfterMs) || P2P_FALLBACK_AFTER_MS));
    degradedAfterMs = Math.min(P2P_DEGRADED_AFTER_MS, Math.floor(fallbackAfterMs * 0.75));
    evaluateHealth();
  }
