...
import { normalizeIceUrl, normalizeTurnServer } from "./p2p-ice-config.mjs";
...
/** Data channel never opens → Firebase fallback. */
export const P2P_CHANNEL_OPEN_TIMEOUT_MS = 30_000;
/** First LOC sent but no valid ACK → Firebase fallback. */
export const P2P_FIRST_ACK_TIMEOUT_MS = 15_000;
/** Retry coalesced pending LOC after backpressure clears. */
export const P2P_BACKPRESSURE_FLUSH_MS = 500;
/** Retry retained pending LOC after channel.send failure (no new GPS required). */
export const P2P_SEND_FAILURE_RETRY_MS = 500;
/** Bounded driver sent-sequence retention for ACK validation. */
...
export function createPeerSessionId(now = Date.now(), cryptoImpl = globalThis.crypto) {
  if (typeof cryptoImpl?.getRandomValues !== "function") throw new Error("SECURE_RANDOM_UNAVAILABLE");
  const rand = Array.from(cryptoImpl.getRandomValues(new Uint8Array(16)))
    .map((b) => b.toString(16).padStart(2, "0")).join("");
  return `ps_${now.toString(36)}_${rand}`;
}
...
export function buildIceServers(opts = {}) {
  ...
  for (const url of stuns) {
    const u = normalizeIceUrl(url, "stun");
    if (u) servers.push({ urls: u });
  }
  const turn = normalizeTurnServer(opts.turn);
 ...
 */
export function resolveIceConfiguration(globalObj = typeof globalThis !== "undefined" ? globalThis : {}, now = Date.now()) {
  ...
  const iceServers = buildIceServers({
    stunUrls,
    turn: !cfg.turnExpiresAtMs || now < cfg.turnExpiresAtMs ? cfg.turn : null,
  });
  return {
    iceServers,
    iceCandidatePoolSize: 0,
    // Relay-only is an explicit QA override; normal operation tries all paths.
    iceTransportPolicy: cfg.iceTransportPolicy === "relay" ? "relay" : "all",
    hasStun: iceServers.some((s) => [s.urls].flat().some((u) => /^stuns?:/i.test(u))),
    hasTurn: iceServers.some((s) => [s.urls].flat().some((u) => /^turns?:/i.test(u))),
  };
}
