...
const firebaseConfig = useEmulators
  ? {
      ...
      appId: "1:123456789012:web:phase2e-partner",
    }
  : prodConfig;
...
if (isFirebaseConfigured()) {
  ...
  if (host === "localhost" || host === "127.0.0.1") {
    ...
    try {
      ...
      console.warn("[SwiftGo] functions emulator", e);
    }
    if (useEmulators) {
      try {
        connectAuthEmulator(auth, "http://127.0.0.1:9099", { disableWarnings: true });
      } catch (e) {
        console.warn("[SwiftGo] auth emulator", e);
      }
      try {
        connectFirestoreEmulator(db, "127.0.0.1", 8080);
      } catch (e) {
        console.warn("[SwiftGo] firestore emulator", e);
      }
      try {
        connectStorageEmulator(storage, "127.0.0.1", 9199);
      } catch (e) {
        console.warn("[SwiftGo] storage emulator", e);
      }
    }
  }
}
